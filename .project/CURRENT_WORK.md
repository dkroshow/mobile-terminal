...
## Recently Completed
- **2026-02-12**: CC status detection rewrite — activity timestamp from tmux (`window_activity`), status bar parsing (`⏵` line for `esc to interrupt`), 1s live sidebar updates from output poll, tool-call card splitting in chat parser, ghost suggestion fix (expanded `sawStatus` range, `isIdle()` checks only status bar). Dashboard poll 3s.
